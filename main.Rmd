...
```{r}
f <- file.path('maldi-spectrum.tsv')
...
plot(X.isotopes$intensity~X.isotopes$m.z, pch = 'x', main = 'Close-up on the intensities for the full mass spectrum', sub = 'The first 5 isotopes.', type = 'l', xlab = 'm/z [ ]', ylab = 'Intensity [ ]', lwd = 2)
...
draw_peptide_mass <- function(all.isotopes=TRUE){
  ...
  hs <- replicate(128, ifelse(!all.isotopes || runif(1)<99.9885/100, 1.007825032241, 2.01410177811))
  ns <- replicate(24, ifelse(!all.isotopes || runif(1)<99.634/100, 14.0030740048, 15.0001088982))
  os <- replicate(22, ifelse(!all.isotopes || runif(1)<99.762/100, 15.99491461956, ifelse(runif(1)<(0.038/ (0.038 + 0.2)), 16.99913170, 17.9991610)))
  return(sum(cs)+sum(hs)+sum(ns)+sum(os)+1) # with unit-protonation
}
n <- 50000
density <- density(replicate(n, draw_peptide_mass()))
density$y <- (density$y / max(density$y) * (max(X.isotopes$intensity) - min(X.isotopes$intensity))) + min(X.isotopes$intensity)
lines(density, main = 'Theoretical full mass spectrogram', xlab = 'm/z [ ]', col = 'red', lwd = 2)

density <- density(replicate(n, draw_peptide_mass(all.isotopes = FALSE)))
density$y <- (density$y / max(density$y) * (max(X.isotopes$intensity) - min(X.isotopes$intensity))) + min(X.isotopes$intensity)
lines(density, main = 'Theoretical partial mass spectrogram', xlab = 'm/z [ ]', col = 'purple', lwd = 2)

legend(1782.25, 1000, legend = c('observed', 'theoretical', 'theoretical partial'), col = c('black', 'red', 'purple'), pch = 1)
#dev.off()
```
